...
def deeplab_v3_plus_generator(num_classes,
    ...
    Args:
      ...
      data_format: The input format ('channels_last', 'channels_first', or None).
        ...
        Only 'channels_last' is supported inputly.
...
def lstm(input):
    with tf.variable_scope('lstm'):
        p_input_list = tf.split(input, input.shape[0], 0)

        cell = SegLSTMCell(input.shape[-1])
        state = cell.zero_state(1, input.shape[1], input.shape[2])
        ...
        with tf.variable_scope("ConvLSTM") as scope:  # as BasicLSTMCell
            for i, p_input_ in enumerate(p_input_list):
                ...
                if i == 0:
                    outs = tf.reshape(t_output,
                                           [1, input.shape[1], input.shape[2], input.shape[3]])
                else:
                    outs = tf.concat([outs, tf.reshape(t_output,
