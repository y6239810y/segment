...
if  hostname == "DESKTOP-0M79B8O":
    W_SIZE = 64
    H_SIZE = 64
    root  = "E:/train_data/multi_task_data_train/"
    root_test = "E:/train_data/multi_task_data_test/"
    ...
    W_SIZE = 64
    H_SIZE = 64
    root = "/media/yankai/file/train_data/multi_task_data_train/"
    root_test = "/media/yankai/file/train_data/multi_task_data_test/"
...
class DenseNet:
    def __init__(self,k):
        ...
        self.layers = [
            ...
            'TRANS_2', 'DENSE_3', 'UPSAMPLE_1', 'CONBINE_1', 'CONV_3', 'ADD_1', 'UPSAMPLE_2', 'CONBINE_2',
            'CONV_4', 'ADD_2', 'UPSAMPLE_3', 'CONBINE_3', 'CONV_5'
        ]
        self.layers_kernels = [
            {"kernel": [5, 5, 5], "stride": 2, "filter": 128, "BN":False},  # CONV_1
            {"pool_way": Max_Pooling_3d, "kernel": [3, 3, 3], "stride": 2},  # POOL_1
            {"kernel": [5, 5, 5], "stride": 1, "filter": 256, "BN":True},  # CONV_2
            ...
            {"kernel": [3, 3, 3], "stride": 1, "filter": 512, "BN":True},  # CONV_3

            {"add_layer": 'CONV_2', "kernel": [3, 3, 3]},  # ADD_1
            {"kernel": [3, 3, 3], "stride": [2, 2, 2], "filter": 64},  # UPSAMPLE_2
            {"add_layer": 'CONV_1', "kernel": [3, 3, 3]},  # CONBINE_2

            {"kernel": [3, 3, 3], "stride": 1, "filter": 128, "BN":True},  # CONV_4

            {"add_layer": 'CONV_1', "kernel": [3, 3, 3]},  # ADD_2

            {"kernel": [3, 3, 3], "stride": [2, 2, 2], "filter": 16},  # UPSAMPLE_3

            {"add_layer": 'INPUT', "kernel": [3, 3, 3]},  # CONBINE_3
            {"kernel": [3, 3, 3], "stride": 1, "filter": 3, "BN":True}  # CONV_5
        ]
        current = self.input
        ...
        self.airway_result, self.artery_result,self.airway_correct_prediction,\
        self.artery_correct_prediction=self.GetResult()
    ...
    def RunAccurary(self,artery_accurary,airway_accurary):
        ...
        self.merged_accurary = tf.summary.merge([self.artery_acc,self.airway_acc])
    ...
    def Test(self,inputs,labels,weights):
        ...
        loss, airway, artery, airway_accurary, artery_accurary, learn_rate, step = self.sess.run(
            (
                self.loss, self.airway_result, self.artery_result,
                self.airway_correct_prediction, self.artery_correct_prediction,
                self.lr,self.global_step),
            feed_dict={self.x: inputs, self.y: labels, self.class_weights: weights})
...
def net_train(file_list,weights):
    ...
    for i, file in enumerate(file_list):
        ...
        if(total_artery_accurary+total_airway_accurary>1):
            a=5
            b=5
            c=5
        else:
            a = (1 - total_artery_accurary)*10 / (2 - total_artery_accurary - total_airway_accurary)
            b = (1 - total_airway_accurary)*10 / (2 - total_artery_accurary - total_airway_accurary)
            c = 1
        weights_mod = [a*100,b*100,c*100]
...
def net_test(file_test_list,weights):
    for i, file in enumerate(file_test_list):
        ...
        for index in data_list:
            ...
            loss, airway, artery, airway_accurary, artery_accurary, learn_rate, step = net.Test(
                data, label,weights)

            out_vtk_artery[c_start:c_end, w_start:w_end, h_start:h_end] = out_vtk_artery[c_start:c_end, w_start:w_end,
                                                                          h_start:h_end] + artery + (artery - 1)
            out_vtk_airway[c_start:c_end, w_start:w_end, h_start:h_end] = out_vtk_airway[c_start:c_end, w_start:w_end,
                                                                          h_start:h_end] + airway + (airway - 1)
...
for times in range(TRAIN_TIME):
    ...
    net_test(file_test_list,weights)
