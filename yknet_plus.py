...
class DenseNet:
    def __init__(self,k):
        ...
        self.layers = [
            ...
            'TRANS_2', 'DENSE_3','TRANS_3', 'DENSE_4', 'UPSAMPLE_1', 'CONBINE_1', 'DENSE_5', 'ADD_1', 'UPSAMPLE_2', 'CONBINE_2',
            'DENSE_6', 'ADD_2', 'UPSAMPLE_3', 'CONBINE_3', 'CONV_3'
        ]
        self.layers_kernels = [
...
for times in range(TRAIN_TIME):
    ...
    for i, file in enumerate(file_list):
        data_array,label_array = get_data(root,file,(i+1)%5)
    ...
    for i, file in enumerate(file_test_list):
        ...
        for data,label,index in data_list:
            ...
            airway_percentage = np.sum(label[:, :, :, 1]) / np.size(label[:, :, :, 1])
            net.is_train = False
            loss, airway, artery, airway_accurary, artery_accurary, learn_rate, step = net.Test(data, label, [1, 1,
                                                                                                               1])
