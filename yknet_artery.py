...
class DenseNet:
    def __init__(self,k):
        ...
        self.layers_kernels = [
            ...
            {"kernel": [1, 1, 1], "stride": 1, "filter": 2}  # CONV_3
        ]
        current = self.input
        ...
        self.artery_result ,self.artery_correct_prediction,=self.GetResult()
    ...
    def GetResult(self):
        with tf.variable_scope("GetResult"):
            ...
            artery_result = tf.cast(tf.argmin(x, 3), tf.float32)
            constant = tf.cast(tf.equal(artery_result, artery_result), tf.float32)
            label_artery,label_bg = tf.split(self.y, [1, 1], axis=3)
        ...
        return artery_result,artery_correct_prediction
    ...
    def RunAccurary(self,artery_accurary):

        self.merged_accurary = tf.summary.merge([self.artery_acc])
    ...
    def Train(self,inputs,labels,weights):
        ...
        _, loss,artery,artery_accurary,learn_rate, result, step = self.sess.run((
            self.train_op, self.loss,self.artery_result,self.artery_correct_prediction,
        ...
        return loss,artery,artery_accurary,learn_rate, step
    ...
    def Test(self,inputs,labels,weights):
        ...
        loss, artery,artery_accurary,learn_rate, result, step = self.sess.run(
            (
                self.loss,  self.artery_result, self.artery_correct_prediction,
    ...
    for i, file in enumerate(file_list):
        ...
        for j,index in enumerate(data_list):
            ...
            if (np.sum(label[:, :, :, 0])> 0):
                ...
                loss, artery,artery_accurary,learn_rate, step = net.Train(data, label,weights)
                out_vtk_artery[c_start:c_end, w_start:w_end, h_start:h_end] = artery
                if(j%1==0):
                    print('step ', step, ' artery_percentage ', artery_percentage,'learning_rate',learn_rate)
                    print(' loss ', loss, ' artery_accurary ', artery_accurary)
                    print("=================================================================================")
        ...
        net.RunAccurary(total_artery_accurary)
        print("=================================================================================")
        print("=================================================================================")
        print("=================================================================================")
        print("time: " + str(times+1) + "  Dicom: " + str(i+1) + " total_artery_accurary: " + str(total_artery_accurary))
        print("\n\n\n")
...
def net_test(file_test_list,weights):
    for i, file in enumerate(file_test_list):
        ...
        for index in data_list:
            ...
            loss, artery,artery_accurary,learn_rate, step = net.Test(data, label,weights)
        ...
        print("=================================================================================")
        print("=================================================================================")
        print("time: " + str(times + 1) + "  test: " + str(i + 1) + " total_artery_accurary: " + str(total_artery_accurary))
        print("\n\n\n")
        if os.path.isdir("./vtk"):
            pass
        else:
