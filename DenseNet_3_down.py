...
class DenseNet:
    def __init__(self,k):
        ...
        self.layers = [
            ...
            'TRANS_2','DENSE_3', 'UPSAMPLE_1', 'CONBINE_1', 'DENSE_4','UPSAMPLE_2','CONBINE_2',
            'DENSE_5','UPSAMPLE_3','CONBINE_3','CONV_3'
        ]
        self.layers_kernels = [
        ...
        tf.summary.scalar('airway_correct_accuracy', self.airway_correct_prediction)
...
for times in range(TRAIN_TIME):
    for i, file in enumerate(file_list):
        data_array,label_array = get_data(root,file,2)
        ...
        for data,label,index in data_list:
            if (np.sum(label[:, :, :, 0] + label[:, :, :, 1]) > 0) or (index[0]*index[3]*index[5]==0):
                ...
                out_vtk_artery[c_start:c_end, w_start:w_end, h_start:h_end] = out_vtk_artery[c_start:c_end,
                                                                              w_start:w_end, h_start:h_end] + artery + (
                                                                                  artery - 1)
                out_vtk_airway[c_start:c_end, w_start:w_end, h_start:h_end] = out_vtk_airway[c_start:c_end,
                                                                              w_start:w_end, h_start:h_end] + airway + (
                                                                                  airway - 1)
